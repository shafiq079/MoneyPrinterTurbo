...
PROMPT_VERSION = "nvidia-poster-ranker-v3"
...
def _prompt(
    ...
    return (
        ...
        "Scores are absolute integers 0..100. Apply this mandatory hierarchy: first identify the narration's required primary subject, material, crop, food, or product; then verify that exact primary entity is visibly identifiable in the candidate; only after that evaluate the visible action or process, setting or context, and visual quality. "
        "A matching action, machine, setting, container, box, package, conveyor, farm, or factory cannot compensate for a wrong, generic, hidden, ambiguous, or absent required primary entity. Similar shapes, colors, equipment, and surroundings do not establish identity. "
        "If the required primary entity is not visibly identifiable, relevance must be 0..30 and mismatch must be 70..100. If a visibly different primary entity is present, relevance must be 0..15 and mismatch must be 85..100. Only a visibly correct primary entity may receive relevance above 50; relevance above 80 additionally requires the narrated action or process to be visibly correct when the narration specifies one. "
        "Relevance measures visible correspondence to the narration. Mismatch measures visible semantic contradiction, including a different subject, material, crop, food, product, machine output, action, or process. Do not treat ambiguity as confirmation and do not infer hidden contents from a box, container, package, machine, or setting. "
        "Visual_quality measures only visible usability and presentation, including clarity, composition, framing, lighting, and technical defects, independently of semantic relevance; polished but unrelated footage can have high visual_quality while having low relevance and high mismatch. "
